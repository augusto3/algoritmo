...
class Arbol(object):
    ...
    def inordenClave(self,clave,bClave):
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.inordenClave(clave,bClave)
            if (self.datos[clave]==bClave):
                print(self.info, self.datos)
            if(self.der is not None):
                self.der.inordenClave(clave,bClave)
    ...
    def postordenClave(self,clave,bClave):
        if(self.info is not None):
            if(self.der is not None):
                self.der.postordenClave(clave,bClave)
            if (self.datos[clave]==bClave):
                print(self.info)
            if(self.izq is not None):
                self.izq.postordenClave(clave,bClave)
    ...
    def conta_criaturas_derrotadas(self, dic):
        if(self.info is not None):
            ...
            if (self.datos['derrotado_por'] is not None):
                if(self.datos['derrotado_por'] in dic):
    ...
    def contadorClave(self,clave,bClave):
        cont = 0
        if(self.info is not None):
            if(self.datos[clave]== bClave):
                cont += 1
            if(self.izq is not None):
                cont += self.izq.contadorClave(clave,bClave)
            if(self.der is not None):
               cont += self.der.contadorClave(clave,bClave)
        return cont